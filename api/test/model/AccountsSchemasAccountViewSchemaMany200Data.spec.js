/*
 * S1 MGMT API
 * SentinelOne Management Console API specification.
 *
 * OpenAPI spec version: 2.1
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 2.4.39
 *
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', '../../src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require('../../src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.S1MgmtApi);
  }
}(this, function(expect, S1MgmtApi) {
  'use strict';

  var instance;

  describe('(package)', function() {
    describe('AccountsSchemasAccountViewSchemaMany200Data', function() {
      beforeEach(function() {
        instance = new S1MgmtApi.AccountsSchemasAccountViewSchemaMany200Data();
      });

      it('should create an instance of AccountsSchemasAccountViewSchemaMany200Data', function() {
        // TODO: update the code to test AccountsSchemasAccountViewSchemaMany200Data
        expect(instance).to.be.a(S1MgmtApi.AccountsSchemasAccountViewSchemaMany200Data);
      });

      it('should have the property numberOfSites (base name: "numberOfSites")', function() {
        // TODO: update the code to test the property numberOfSites
        expect(instance).to.have.property('numberOfSites');
        // expect(instance.numberOfSites).to.be(expectedValueLiteral);
      });

      it('should have the property coreSites (base name: "coreSites")', function() {
        // TODO: update the code to test the property coreSites
        expect(instance).to.have.property('coreSites');
        // expect(instance.coreSites).to.be(expectedValueLiteral);
      });

      it('should have the property completeSites (base name: "completeSites")', function() {
        // TODO: update the code to test the property completeSites
        expect(instance).to.have.property('completeSites');
        // expect(instance.completeSites).to.be(expectedValueLiteral);
      });

      it('should have the property updatedAt (base name: "updatedAt")', function() {
        // TODO: update the code to test the property updatedAt
        expect(instance).to.have.property('updatedAt');
        // expect(instance.updatedAt).to.be(expectedValueLiteral);
      });

      it('should have the property totalComplete (base name: "totalComplete")', function() {
        // TODO: update the code to test the property totalComplete
        expect(instance).to.have.property('totalComplete');
        // expect(instance.totalComplete).to.be(expectedValueLiteral);
      });

      it('should have the property expiration (base name: "expiration")', function() {
        // TODO: update the code to test the property expiration
        expect(instance).to.have.property('expiration');
        // expect(instance.expiration).to.be(expectedValueLiteral);
      });

      it('should have the property isDefault (base name: "isDefault")', function() {
        // TODO: update the code to test the property isDefault
        expect(instance).to.have.property('isDefault');
        // expect(instance.isDefault).to.be(expectedValueLiteral);
      });

      it('should have the property controlSites (base name: "controlSites")', function() {
        // TODO: update the code to test the property controlSites
        expect(instance).to.have.property('controlSites');
        // expect(instance.controlSites).to.be(expectedValueLiteral);
      });

      it('should have the property agentsInCoreSku (base name: "agentsInCoreSku")', function() {
        // TODO: update the code to test the property agentsInCoreSku
        expect(instance).to.have.property('agentsInCoreSku');
        // expect(instance.agentsInCoreSku).to.be(expectedValueLiteral);
      });

      it('should have the property state (base name: "state")', function() {
        // TODO: update the code to test the property state
        expect(instance).to.have.property('state');
        // expect(instance.state).to.be(expectedValueLiteral);
      });

      it('should have the property id (base name: "id")', function() {
        // TODO: update the code to test the property id
        expect(instance).to.have.property('id');
        // expect(instance.id).to.be(expectedValueLiteral);
      });

      it('should have the property creatorId (base name: "creatorId")', function() {
        // TODO: update the code to test the property creatorId
        expect(instance).to.have.property('creatorId');
        // expect(instance.creatorId).to.be(expectedValueLiteral);
      });

      it('should have the property accountType (base name: "accountType")', function() {
        // TODO: update the code to test the property accountType
        expect(instance).to.have.property('accountType');
        // expect(instance.accountType).to.be(expectedValueLiteral);
      });

      it('should have the property unlimitedComplete (base name: "unlimitedComplete")', function() {
        // TODO: update the code to test the property unlimitedComplete
        expect(instance).to.have.property('unlimitedComplete');
        // expect(instance.unlimitedComplete).to.be(expectedValueLiteral);
      });

      it('should have the property salesforceId (base name: "salesforceId")', function() {
        // TODO: update the code to test the property salesforceId
        expect(instance).to.have.property('salesforceId');
        // expect(instance.salesforceId).to.be(expectedValueLiteral);
      });

      it('should have the property unlimitedControl (base name: "unlimitedControl")', function() {
        // TODO: update the code to test the property unlimitedControl
        expect(instance).to.have.property('unlimitedControl');
        // expect(instance.unlimitedControl).to.be(expectedValueLiteral);
      });

      it('should have the property totalControl (base name: "totalControl")', function() {
        // TODO: update the code to test the property totalControl
        expect(instance).to.have.property('totalControl');
        // expect(instance.totalControl).to.be(expectedValueLiteral);
      });

      it('should have the property skus (base name: "skus")', function() {
        // TODO: update the code to test the property skus
        expect(instance).to.have.property('skus');
        // expect(instance.skus).to.be(expectedValueLiteral);
      });

      it('should have the property totalLicenses (base name: "totalLicenses")', function() {
        // TODO: update the code to test the property totalLicenses
        expect(instance).to.have.property('totalLicenses');
        // expect(instance.totalLicenses).to.be(expectedValueLiteral);
      });

      it('should have the property usageType (base name: "usageType")', function() {
        // TODO: update the code to test the property usageType
        expect(instance).to.have.property('usageType');
        // expect(instance.usageType).to.be(expectedValueLiteral);
      });

      it('should have the property totalCore (base name: "totalCore")', function() {
        // TODO: update the code to test the property totalCore
        expect(instance).to.have.property('totalCore');
        // expect(instance.totalCore).to.be(expectedValueLiteral);
      });

      it('should have the property billingMode (base name: "billingMode")', function() {
        // TODO: update the code to test the property billingMode
        expect(instance).to.have.property('billingMode');
        // expect(instance.billingMode).to.be(expectedValueLiteral);
      });

      it('should have the property agentsInCompleteSku (base name: "agentsInCompleteSku")', function() {
        // TODO: update the code to test the property agentsInCompleteSku
        expect(instance).to.have.property('agentsInCompleteSku');
        // expect(instance.agentsInCompleteSku).to.be(expectedValueLiteral);
      });

      it('should have the property externalId (base name: "externalId")', function() {
        // TODO: update the code to test the property externalId
        expect(instance).to.have.property('externalId');
        // expect(instance.externalId).to.be(expectedValueLiteral);
      });

      it('should have the property unlimitedExpiration (base name: "unlimitedExpiration")', function() {
        // TODO: update the code to test the property unlimitedExpiration
        expect(instance).to.have.property('unlimitedExpiration');
        // expect(instance.unlimitedExpiration).to.be(expectedValueLiteral);
      });

      it('should have the property licenses (base name: "licenses")', function() {
        // TODO: update the code to test the property licenses
        expect(instance).to.have.property('licenses');
        // expect(instance.licenses).to.be(expectedValueLiteral);
      });

      it('should have the property activeAgents (base name: "activeAgents")', function() {
        // TODO: update the code to test the property activeAgents
        expect(instance).to.have.property('activeAgents');
        // expect(instance.activeAgents).to.be(expectedValueLiteral);
      });

      it('should have the property agentsInControlSku (base name: "agentsInControlSku")', function() {
        // TODO: update the code to test the property agentsInControlSku
        expect(instance).to.have.property('agentsInControlSku');
        // expect(instance.agentsInControlSku).to.be(expectedValueLiteral);
      });

      it('should have the property name (base name: "name")', function() {
        // TODO: update the code to test the property name
        expect(instance).to.have.property('name');
        // expect(instance.name).to.be(expectedValueLiteral);
      });

      it('should have the property createdAt (base name: "createdAt")', function() {
        // TODO: update the code to test the property createdAt
        expect(instance).to.have.property('createdAt');
        // expect(instance.createdAt).to.be(expectedValueLiteral);
      });

      it('should have the property creator (base name: "creator")', function() {
        // TODO: update the code to test the property creator
        expect(instance).to.have.property('creator');
        // expect(instance.creator).to.be(expectedValueLiteral);
      });

      it('should have the property unlimitedCore (base name: "unlimitedCore")', function() {
        // TODO: update the code to test the property unlimitedCore
        expect(instance).to.have.property('unlimitedCore');
        // expect(instance.unlimitedCore).to.be(expectedValueLiteral);
      });

    });
  });

}));
